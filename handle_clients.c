...
void init_commands(command_entry_t *commands)
{
    ...
    commands[9] = (command_entry_t){"LIST", list_handling};
    commands[10] = (command_entry_t){"PASV", pasv_handling};
    commands[11] = (command_entry_t){"PORT", port_handling};
    commands[12] = (command_entry_t){NULL, NULL};
}

static bool is_valid_command(const char *buffer, const char *cmd, int len)
{
    return (strncasecmp(buffer, cmd, len) == 0 &&
        (isspace((unsigned char)buffer[len]) || buffer[len] == '\0'));
}

static void check_command(command_t *cmd)
{
    command_entry_t commands[13];
